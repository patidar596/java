...
<html>
<head>
<link href="<c:url value="/resources/styles/faq.css" />"
	rel="stylesheet">
<meta charset="UTF-8">
<style>
.home {
				...
				<c:forEach items="${pending_questions}" var="item">
					<div>
						...
						<label for="question9" class="question">
							${item.getQuestion()} <a
							href="<c:url value="/emp/del_q/${item.getQ_id()}"/>">delete</a>
						</label>

						<form class="answers"
							action="<c:url value='/emp/ans_q/${item.getQ_id()}'/>" id="form">
							<p>
								<input type="text" placeholder="" id="Answer" name="Answer"
									size="300" />
							</p>
							<p>
